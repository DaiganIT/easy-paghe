...
test('getHiredPeople has the correct url', () => {
  ...
  jest.useFakeTimers();
  const promise = http.getHiredPeople({ search, page, pageLimit });
  jest.runAllTimers();

  expect(axiosMock.get.mock.calls.length).toEqual(1);
  expect(axiosMock.get.mock.calls[0][0]).toEqual('/api/hired?filter=hi&page=1&pageLimit=10');
  expect(axiosMock.get.mock.calls[0][1]).toEqual({ cancelToken: 'token' });
  expect(promise[1]).toBeTruthy();
});

test('getHiredPeople is called just once inside the same timer window', () => {
  axiosMock.get.mockResolvedValueOnce();
  const search = 'hi';
  const page = 0;
  const pageLimit = 10;
  ...
  jest.useFakeTimers();
  const promise = http.getHiredPeople({ search, page, pageLimit });
  http.getHiredPeople({ search, page, pageLimit });
  http.getHiredPeople({ search, page, pageLimit });
  jest.runAllTimers();
